...
class DistillationTrainer(SFTTrainer):
    ...
    def compute_kl_loss(self, student_logits, ensemble_logits, mask, inputs, temperature=1.0):
        ...
        # Compute KL Loss
        # -----------------------
        if config.use_selected_logits_kl:
            # FSDP2 / memory-efficient KL divergence for selected logits
            student_probs = F.log_softmax(student_logits / temperature, dim=-1)
            student_masked_probs = student_probs[mask]  # [valid_count, vocab_size]

            teacher_logprob_values = torch.cat([torch.tensor(vals) for vals in inputs['logprob_values']], dim=0).to(student_logits.device)
            teacher_logprob_indices = torch.cat([torch.tensor(inds) for inds in inputs['logprob_indices']], dim=0).to(torch.int64).to(student_logits.device)
            
            student_selected_probs = student_masked_probs.gather(dim=-1, index=teacher_logprob_indices)
            kl_loss = F.kl_div(student_selected_probs, teacher_logprob_values, log_target=True, reduction="sum")
            return kl_loss / mask.sum() # Normalize by number of valid tokens

        else:
            # Original / full logit reconstruction KL divergence
